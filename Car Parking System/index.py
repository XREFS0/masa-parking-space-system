...
class Parking:
    ...
    def start(self):
        ...
        print(colored("Thanks for using car parking system", "green"))

    def _park_vehicle(self):
        vehicle_type = self._get_safe_int(
            "Available vehicle types: 1. Car\t2. Bike\t3. Truck.\nEnter your choice: "
        )
    ...
    def show_layout(self):
        ...
        print(colored(f"|{''.join(col_info)}|columns", "magenta"))
        ...
        for i, row in enumerate(self.slots, 1):
            ...
            for j, col in enumerate(row, 1):
                string_to_printed += colored(
                    f"[{col.vehicle if col.vehicle else ' '}]", "red" if col.vehicle else "green"
                )
...
def main():
    try:
        print(colored("First set your parking lot", "yellow"))
